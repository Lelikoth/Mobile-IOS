...
def pay():
    ...
    if not name or not card_number or not expiry or not cvv or amount is None:
        return jsonify({"message": "Missing fields"}), 400

    digits = "".join([c for c in card_number if c.isdigit()])

    # Mock zasada: karta kończy się na 0 => fail, inaczej success
    if digits.endswith("0"):
        return jsonify({
            "status": "failed",
            "message": "Payment rejected by mock-bank.",
            "paymentId": None
        }), 200

    payment_id = "PAY_" + secrets.token_hex(6).upper()
    return jsonify({
        "status": "success",
        "message": f"Payment received: {amount}",
